...
const REPORT_TYPES = [
  { id: 'cost-per-head', name: 'ต้นทุน/คน/วัน', desc: 'เมนู → เบิก → ÷ พนักงานวันนั้น (Output หลัก)' },
  { id: 'movement', name: 'รายงานเคลื่อนไหววัตถุดิบ', desc: 'รับเข้า / เบิก / ปรับยอด รายวัน' },
  { id: 'valuation', name: 'มูลค่าสต็อกคงคลัง', desc: 'แยกตามหมวดวัตถุดิบ' },
  { id: 'menu-cost', name: 'ต้นทุนตามเมนู', desc: 'BOM วัตถุดิบต่อเมนู × รอบเบิก' },
...
export default function ReportsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        title="ต้นทุน & รายงาน"
        subtitle="ต้นทุน/คน/วัน · ต้นทุนตามเมนู · Export Excel / PDF"
      ...
      <div className="mb-5 grid grid-cols-2 gap-3.5 lg:grid-cols-4">
        <KpiCard label="ต้นทุน/คน/วัน" value="—" hint="Output หลักของโรงครัว" accent="#2f9e6b" />
        <KpiCard label="ต้นทุนเบิกเดือนนี้" value={fmtMoney(kpi?.monthlyWithdrawCost ?? 0)} accent="#2a7de1" />
        <KpiCard label="มูลค่าสต็อกวัตถุดิบ" value={fmtMoney(kpi?.totalStockValue ?? 0)} accent="#64748b" />
        <KpiCard label="รอซิงก์" value={String(kpi?.pendingSyncCount ?? 0)} accent="#d6493b" />
      </div>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <div className="mb-4 font-bold text-sm">ต้นทุนตามเมนู (รอบเบิก)</div>
          {abcLoading ? (
