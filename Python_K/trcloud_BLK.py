...
DEFAULT_OUTPUT = os.path.join(DATA_DIR, "bl.json")
# ============================================================
...
class TRCloudBillFetcher:
    ...
    def export_to_json(self, records: list, items: list, filename: str,
        ...
        """บันทึก .json — orders (bill headers) + lines (items)"""
        if not records and not items:
            print("⚠️ ไม่มีข้อมูลสำหรับบันทึก")
            return
        out_dir = os.path.dirname(filename)
        if out_dir:
            os.makedirs(out_dir, exist_ok=True)
        orders = self.format_dataframe(records)
        payload = {
        ...
        try:
            ...
            print(f"📝 orders: {len(orders)} · lines: {len(items)}")
...
if __name__ == "__main__":
    ...
    if args.format == 'xlsx':
        out = args.output if args.output.lower().endswith('.xlsx') else os.path.splitext(args.output)[0] + '.xlsx'
        fetcher.export_to_excel(records, items, out)
    elif args.format == 'csv':
        fetcher.export_to_csv(records, items, args.output)
    else:
        out = args.output if args.output.lower().endswith('.json') else os.path.splitext(args.output)[0] + '.json'
        fetcher.export_to_json(records, items, out, args.date_from, args.date_to)
