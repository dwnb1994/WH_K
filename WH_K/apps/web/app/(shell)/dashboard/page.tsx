...
const CATEGORIES = [
  { name: 'ฮาร์ดแวร์ / น็อต', pct: 42, color: '#18181b' },
  { name: 'เครื่องมือไฟฟ้า', pct: 28, color: '#2a7de1' },
  { name: 'วัสดุสิ้นเปลือง', pct: 18, color: '#2f9e6b' },
  { name: 'อื่น ๆ', pct: 12, color: '#d99a2b' },
]
...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        title="ภาพรวมคลังโรงครัว"
        subtitle={`${new Date().toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' })} · อัปเดตล่าสุด ${new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}`}
        actions={
          <>
      ...
      <div className="mb-5 grid grid-cols-1 gap-3.5 sm:grid-cols-2 xl:grid-cols-4">
        {isLoading
          ...
          : (
            <>
              <KpiCard label="มูลค่าสต็อกรวม" value={fmtMoney(kpi?.totalStockValue ?? 0)} hint="▲ จากสัปดาห์ก่อน" hintTone="good" />
              <KpiCard label="ต้นทุนเบิกเดือนนี้" value={fmtMoney(kpi?.monthlyWithdrawCost ?? 0)} hint="จาก Cost Snapshot" />
              <KpiCard label="ต่ำกว่าจุดสั่งซื้อ" value="—" hint="ตรวจจาก Master data" hintTone="warn" />
              <KpiCard
        ...
        <Card className="p-5">
          <div className="mb-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14.5px] font-bold">จ่ายออกวันนี้</span>
            </div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-3.5 lg:grid-cols-5">
        ...
        <Card className="col-span-2 p-5">
          <div className="mb-4 text-[14.5px] font-bold">สัดส่วนตามหมวด</div>
