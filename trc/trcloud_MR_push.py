# -*- coding: utf-8 -*-
"""
TRCloud MR Pusher — สร้าง/บันทึกใบเบิกสินค้า (Material Request)

Endpoint:
  POST /application/ics/api/engine-ics/edit_mr.php
  Body: json={...}  (json-wrap, id ว่าง = สร้างใหม่)

วิธีรัน:
  py trcloud_MR_push.py --input templates/mr_push.json --dry-run
  py trcloud_MR_push.py --input my_mr.json
"""
...
API_SAVE = f"{BASE_URL}/application/ics/api/engine-ics/edit_mr.php"
REFERER = f"{BASE_URL}/application/ics/material_request.php"
...
class TRCloudMRPusher:
    ...
    def build_payload(self, doc: dict) -> dict:
        ...
        mr_id = str(doc.get("mr_id") or doc.get("id") or "")
        ...
        payload: dict[str, Any] = {
            **auth_fields(self.company_id, self.passkey, self.cookie, self.user_id),
            ...
            "staff": doc.get("staff") or doc.get("salesman") or "",
            "salesman": doc.get("salesman") or doc.get("staff") or "",
            "department": doc.get("department") or "",
            "project": doc.get("project") or "",
            "warehouse": doc.get("warehouse") or "",
            "client_name": doc.get("client_name") or "",
            "client_telephone": doc.get("client_telephone") or "",
    ...
    def push_one(self, doc: dict, dry_run: bool = False) -> dict:
        ...
        label = payload.get("document_number") or payload.get("id") or "new MR"
        session = make_session(self.cookie, REFERER)

        if dry_run:
            print(f"🔍 [dry-run] MR {label}: {len(payload['list'])} รายการ")
            return {"success": True, "dry_run": True, "label": label, "payload": payload}

        data = post_json_wrap(session, API_SAVE, payload)
        if is_success(data):
            print(f"✅ บันทึก MR สำเร็จ: {label} — {data.get('message', '')}")
        else:
            print(f"❌ MR ไม่สำเร็จ: {label} — {data.get('message', data)}")
        return data


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TRCloud MR (ใบเบิก) pusher")
    parser.add_argument("--company-id", default=COMPANY_ID)
    parser.add_argument("--passkey", default=PASSKEY)
    parser.add_argument("--cookie", default=None)
    parser.add_argument("--user-id", default="")
    parser.add_argument("--input", required=True, help="JSON ใบเบิก (ดู templates/mr_push.json)")
    ...
    pusher = TRCloudMRPusher(args.company_id, args.passkey, cookie, user_id=args.user_id)
    push_many("MR push", docs, pusher.push_one, dry_run=args.dry_run, delay=args.delay)
